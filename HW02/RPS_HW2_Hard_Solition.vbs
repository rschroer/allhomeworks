...
'the headers after the fact
    ...
    sht.Range("O4").Value = "Greatest Volume"
    sht.Range("p1").Value = "Ticker"
    sht.Range("Q1").Value = "Value"
    sht.Range("O:Q").ColumnWidth = 17
    
 'find the max value in the range
    MaxChange = Application.WorksheetFunction.Max(sht.Range("K2:K" & Lastrow))
    sht.Range("Q2") = MaxChange
    sht.Range("Q2").NumberFormat = "0.00%"
    sht.Range("P2") = "= Index( I:I , Match( Q2, K:K, 0))"
    
    
'Find the Min Value in the range
    MinChange = Application.WorksheetFunction.Min(sht.Range("K2:K" & Lastrow))
    sht.Range("Q3") = MinChange
    sht.Range("Q3").NumberFormat = "0.00%"
    sht.Range("P3") = "= Index( I:I , Match( Q3, K:K, 0))"

'Find the Max volume
    MaxVolume = Application.WorksheetFunction.Max(sht.Range("L2:L" & Lastrow))
    sht.Range("Q4") = MaxVolume
    sht.Range("P4") = "= Index( I:I , Match( Q4, L:L, 0))"
